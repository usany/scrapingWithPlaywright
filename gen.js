...
bldgs.forEach(([name, start, end]) => {
  const kd = ko.slice(start, end + 1).filter(l => l.trim() && !l.startsWith('#')).map(l => `  "${l.slice(0, l.indexOf('\n')).replace(/\t/g, '\\t')}"`);
  const ed = en.slice(start, end + 1).filter(l => l.trim() && !l.startsWith('#')).map(l => `  "${l.slice(0, l.indexOf('\n')).replace(/\t/g, '\\t')}"`);
